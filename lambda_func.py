...
def upload_image(img_data, bucket, url=None):
    ...
    if auth_is_valid(event):
        ...
        if event["httpMethod"] == "POST" and event["path"] == "/change-password":
            ...
            try:
                if authLevel == "200":
                    exec_statement = (
                        'SELECT * FROM "blog-users" where userStatus=\'active\' and name = \''
                        + name
                        + "' and authLevel in ('200');"
                    )
                elif authLevel == "100":
                    exec_statement = (
                        'SELECT * FROM "blog-users" where userStatus=\'active\' and name = \''
            ...
            bucket = "production-blog-qi-bucket-681409311706"
            ...
            try:
                if authLevel == "200":
                    exec_statement = """SELECT * FROM "blog-users" where userStatus=\'active\' and authLevel  = '200';"""
                elif authLevel == "100":
                    exec_statement = """SELECT * FROM "blog-users" where userStatus=\'active\' and authLevel in ('200','100');"""
                else:
                    exec_statement = """SELECT * FROM "blog-users" where userStatus=\'active\';"""
